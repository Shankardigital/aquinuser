...
function Advertisements() {
  ...
  const Ads = () => {
    ...
    axios
      .post(
        "https://aquinapi.aquin.us/api/v1/web/categ/getall-activeadcatgoeries",
        {},
        {
      ...
      .then((res) => {
        if (res.status == 200) {
          setusers(res.data.adCategResult);
        }
      },
      (error) => {
  ...
  const Adds = (data,i) => {
    ...
    axios
      .post(
        "https://aquinapi.aquin.us/api/v1/web/categ/getcateg",
        params,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      .then((res) => {
        if (res.status == 200) {
          // setusers0(res.data.getSubcatResult);
          // console.log(users0);
          console.log(res.data)
          navigate("/sub-ads");
          sessionStorage.setItem("categoryId", res.data.categrResult._id);
          // sessionStorage.setItem("adscatid", res.data.getSubcatResult.title)
        }
      },
      (error) => {
        if (error.response && error.response.status === 400) {
    ...
    // navigate("/plan-ads",navigate("/plan-ads", { state: { categoryId: data._id} });
  };
